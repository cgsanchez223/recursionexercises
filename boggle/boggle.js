...
function findFrom(board, word, y, x, seen) {
  /* Can we find a word on a board, starting at x, y? */

  if (board[y][x] != word[0]) return false; // not letter we are looking for

  if (seen.has(y + "-" + x)) return false; // Have seen letter before in current path

  if (word.length === 1) return true; // Down to last letter, winner
